...
const EditAddress = () => {
  ...
  const getAddressDetails = useCallback(
    (id) => {
      ...
      axios.post("address-edit", data).then((res) => {
        dispatch(toggleLoader());
        if (res?.data) {
  ...
  useEffect(() => {
    setAddId(router?.query?.id);
    setTimeout(() => {
      window.scrollTo(0, 0);
    }, 0);
    getCountryList();
    getAddressDetails(router?.query?.id);
  }, [router?.query?.id, getAddressDetails, getCountryList]);
  ...
  const initialValues = {
    ...
    default: address?.default_shipping_address === "Y" ? true : false,
  ...
  const onSubmit = (values, actions) => {
    ...
    dispatch(toggleLoader());
    axios.post("update-address", data).then((res) => {
      dispatch(toggleLoader());
      if (res.data.result) {
  ...
  return (
    <>
      <Head>
        <title>Edit Address || Fast Buy</title>
        <meta name="description" content="Edit Address || Fast Buy"></meta>
      </Head>
      <ProfileLayout>
...
export default userRoutes(EditAddress);
